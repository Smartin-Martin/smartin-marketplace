...
Catch your raw ideas as they come. See which ones are worth checking. Get one clean weekly Finding worth forwarding.
...
- `/compass` shows the state of your ideas. What is caught, what is maturing, what is worth checking next.
- `/finding` produces this week's Finding now.
- `/kompas` generates a full workshop set for one mature idea, ready to verify it properly.
- `/kompas-review` reviews an existing workshop set and flags issues.

You do not need the commands for everyday use. Just talk. Compass catches and sorts on its own.
...
No setup, no environment variables. Install, open, start talking. A storage folder is created with a sensible default. You can change it any time.
